...
function updateNumber(el, oldV, newV, fixed){
  ...
  for(let i=0;i<newStr.length;i++){
    ...
    if(newChar!==oldChar){
      if(newV>oldV) span.classList.add("up");
      else if(newV<oldV) span.classList.add("down");
      setTimeout(()=>span.classList.remove("up","down"),500);
    }
    el.appendChild(span);
  }
}
...
function animate(){
  ...
  displayedPrice+= (targetPrice-displayedPrice)*0.15; // più sensibile
  ...
  // Seconda linea mobile
  priceLineCurrentEl.style.left=( (displayedPrice-price24hLow)/range*100 )+"%";
