...
// Creiamo linea gialla se non esiste
let lineEl = document.querySelector(".price-line-current");
if(!lineEl){
    lineEl = document.createElement("div");
    lineEl.className = "price-line-current";
    document.querySelector(".price-bar-container").appendChild(lineEl);
}
...
// Aggiorna numero solo se cambia
function updateNumber(el, oldV, newV, fixed){
    const strOld = oldV.toFixed(fixed);
    const strNew = newV.toFixed(fixed);
    if(strOld !== strNew){
        el.innerText = strNew;
...
async function loadData(){
    ...
    try{
        ...
        console.error("Errore caricamento dati Injective:", e);
    }
}
...
function animate(){
    ...
    const percentChange = ((displayedPrice - price24hOpen)/range)*50;

    // Larghezza e posizione barra
    priceBarEl.style.left = center + Math.min(percentChange,0) + "%";
    priceBarEl.style.width = Math.abs(percentChange) + "%";
    priceBarEl.style.background = displayedPrice >= price24hOpen ? "#22c55e" : "#ef4444";

    // Linea gialla trascina la barra
    lineEl.style.left = center + percentChange + "%";
