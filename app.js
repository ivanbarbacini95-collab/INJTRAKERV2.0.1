...
// Price
let displayedPrice = 0, targetPrice = 0, priceOpen = 0;
...
async function fetchCandles(tf){
  ...
  try{
    ...
    priceOpen=chartData[0].o;
    priceLow=Math.min(...chartData.map(c=>c.l));
    priceHigh=Math.max(...chartData.map(c=>c.h));
...
function drawCandlestick(){
  ...
  chart=new Chart(ctx,{
    ...
    options:{
      ...
      scales:{ x:{ type:'time', time:{ unit:selectedTF }, ticks:{color:'#9ca3af'}, grid:{color:'#1e293b'} },
...
function startWS(){
  ...
  ws.onmessage=e=>{
    ...
    if(last){ last.h=Math.max(last.h,p); last.l=Math.min(last.l,p); last.c=p; }
  };
...
function animate(){
  ...
  // Price Bar
  const center=50;
  const percent=Math.min(Math.abs(displayedPrice-priceOpen)/Math.max(priceHigh-priceLow,0.0001)*50,50);
  let linePos=displayedPrice>=priceOpen?center+percent:center-percent;
  priceBarEl.style.left=`${displayedPrice>=priceOpen?center:linePos}%`;
  priceBarEl.style.width=`${Math.abs(linePos-center)}%`;
  priceBarEl.style.background=displayedPrice>=priceOpen?
...
function colorDigits(newVal, oldVal){
  ...
  for(let i=0;i<newStr.length;i++){
    const c=newStr[i], oldC=oldStr[i]||"";
    if(c!==oldC && /\d/.test(c)){
